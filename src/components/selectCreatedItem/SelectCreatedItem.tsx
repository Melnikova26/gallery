...
const SelectCreatedItem: React.FC<ISelectCreatedProps> = ({ name }) => {
  ...
  const [valuesInput, setValuesInput] = useState<IValuesInput>({
    from: "",
    to: "",
  });
  ...
  useEffect(() => {
    const optionValue =
      valuesInput.from !== "" || valuesInput.to !== ""
        ? {
            label: `${valuesInput.from}-${valuesInput.to}`,
            value: `${valuesInput.from}-${valuesInput.to}`,
          }
        : null;
