...
const SelectItem: React.FC<ISelectItemProps> = ({name, options}) => {
    ...
    const customStyles: StylesConfig = {
        control: (provided, state) => ({
          ...
          border: state.isFocused || state.menuIsOpen ? '1px solid #000!important' : '1px solid rgba(0, 0, 0, .30)',
...
return (
    <>
        <Select styles={customStyles} className={`${st.select}`} placeholder={name} isSearchable={true} components={{DropdownIndicator}} options={options} onMenuOpen={() => setIsDropdownOpen(true)}
